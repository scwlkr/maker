...
`tools.py` defines native tool schemas and executes tool calls. `shell` and
`run_script` run in the sandbox. `search` and `fetch` run from the controller
process. `fetch` performs public URL checks before network access.

## Maker Place

`maker_place.py` manages local observation files, event append, wake locks,
snapshots, wake summaries, field notes, optional raw output storage, and
timestamps.

...
| `TOOL_SCHEMA_MODE` | `all` | `all` exposes all native tools; `shell-only` exposes only `shell`; `script-only` exposes only `run_script`; `write-only` exposes only `write_file`; `files` exposes `list_files`, `read_file`, `write_file`, and `append_file`; `code` exposes the file tools plus `run_script` |
